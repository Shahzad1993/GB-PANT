...
<head>
        <meta charset="utf-8" />
        <title>Conductor Self Appraisal | UKPN</title>
												...
												<table class="table table-bordered">
													...
													<tbody>
														...
														if($_SESSION['astro_role']=='Admin'){
															$query3="select present_post,year,reporting_authority_work_location,reporting_office_name,work_location,office_name from acr";
														}else{
															//$query3="select present_post,year,reporting_authority_work_location,reporting_office_name,work_location,office_name from acr where reporting_authority_name='".$_SESSION['astro_email']."' and work_location='".$record31['work_location']."' and office_name='".$record31['office_name']."'";
															$query3="select present_post,year,reporting_authority_work_location,reporting_office_name,work_location,office_name from acr where reporting_authority_name='".$_SESSION['astro_email']."'";
														}
														$row3=$db->select($query3);
														if($row3->num_rows > 0){
															while($record3=$row3->fetch_array()){
																$query="SELECT id,post_name FROM `post` WHERE id = '".$record3['present_post']."' and post_name_en='CONDUCTOR'";
																$row=$db->select($query);
																if($row->num_rows > 0){
																		    ...
																		    while($record1=$row1->fetch_array()){
    																			if($_SESSION['astro_role']!='Admin'){
    																				...
    																				if($row2->num_rows > 0){
    																					...
    																					$create_button = '<a href="driver-conductor-self-appraisal?id='.$record1['employee_code'].'" class="btn btn-success btn-sm">Create Conductor Self Appraisal</a>';
    																				}
    																			}else{
    																			    $create_button = '';
    																			}
