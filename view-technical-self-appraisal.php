...
<head>
        <meta charset="utf-8" />
        <title>View Technical Self Appraisal | UKPN</title>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    <h4 class="page-title">View Technical List</h4>
                                    <div class="page-title-right">
                                        <ol class="breadcrumb m-0">
                                            <li class="breadcrumb-item"><a href="javascript: void(0);">Technical Self Appraisal</a></li>
                                            <li class="breadcrumb-item active">Technical List</li>
                                        </ol>
                                    </div>
                                </div>
                            </div>
												...
												<table class="table table-bordered">
													...
													<tbody>
														...
														if($_SESSION['astro_role']=='Admin'){
															...
															//echo $query3="select present_post,year from acr where reporting_authority_name='".$_SESSION['astro_email']."' and work_location='".$record31['work_location']."' and office_name='".$record31['office_name']."'";
														...
														if($row3->num_rows > 0){
															while($record3=$row3->fetch_array()){
																$query="SELECT * FROM `post` WHERE id = '".$record3['present_post']."'";
																$row=$db->select($query);
																if($row->num_rows > 0){
																	...
																	if($record['level']=='10'){
																		...
																		if($row1->num_rows > 0){
																		    while($record1=$row1->fetch_array()){
    																			if($_SESSION['astro_role']!='Admin'){
    																				$query2="select * from self_appraisal where year='".$record3['year']."' and employee='".$record1['employee_code']."'";
    																				$row2=$db->select($query2);
    																				if($row2->num_rows > 0){
    																					$record2=$row2->fetch_array();
    																					$create_button = '<a href="view-self-appraisal-details?id='.$record2['id'].'" class="btn btn-success btn-sm">View Self Appraisal Details</a>';
    																				}
    																			}else{
    																			    $create_button = '';
    																			}
