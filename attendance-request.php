...
<head>
	<meta charset="utf-8" />
	<title>Attendance Request | UKPN</title>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    <h4 class="page-title">Attendance Request</h4>
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <div class="row pt-2">
											<div class="col-lg-4">
												<input type="date" value="<?= date("Y-m-d"); ?>" onchange="action13(this.value)" class="form-control">
											</div>
										</div>
													...
													<div class="table-responsive">
														<table class="table table-sm mb-0">
															...
															<tbody id="attendance_request_data">
																...
																$query="select * from regulization_request where requested_on='".date("Y-m-d")."'";
																$row=$db->select($query);
																if ($row->num_rows > 0) {
																	while($record=$row->fetch_array()){
																		if($_SESSION['astro_role']=='Division'){
																			...
																			$sql1 = "SELECT * FROM `employee` where employee_code='".$record['employee']."' and work_location='Division' and office_name='".$record11['id']."' and (attendance_type!='Manual' or attendance_type!=NULL)";
																		}else if($_SESSION['astro_role']=='Deport'){
																			$query11="select * from deport where phone='".$_SESSION['astro_email']."'";
																			$row11=$db->select($query11);
																			$record11=$row11->fetch_array();
																			
																			$sql1 = "SELECT * FROM `employee` where employee_code='".$record['employee']."' and work_location='Deport' and office_name='".$record11['id']."' and (attendance_type!='Manual' or attendance_type!=NULL)";
																		}else if($_SESSION['astro_role']=='Head Quarter'){
																			$sql1 = "SELECT * FROM `employee` where employee_code='".$record['employee']."' and work_location='Head Quarter' and (attendance_type!='Manual' or attendance_type!=NULL)";
																		}else{
																			$sql1 = "SELECT * FROM `employee` WHERE employee_code='".$record['employee']."'";
																		}
																		...
																		if ($exe1->num_rows > 0) {
																			...
																			$sql2 = "SELECT * FROM `login` WHERE employee_code='".$record['action_taken_by']."'";
																			$exe2 = $db->select($sql2);
																			if ($exe2->num_rows > 0) {
																			...
																			$output .='</td>
																				...
																				<td>'.date("d M, Y",strtotime($record['requested_on'])).'
																					'.date("h:i A",strtotime($record['requested_on_time'])).'
