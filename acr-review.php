...
$query3="select * from acr where id='".$_REQUEST['id']."' and reviewing_authority_name='".$_SESSION['astro_email']."'";
$row3=$db->select($query3);
if($row3->num_rows > 0){
	$record3=$row3->fetch_array();
}else{
	//header("Location: appraisal-list");
}

$query="select * from employee_appraisal where id='".$_REQUEST['employee-appraisal-id']."'";
$row=$db->select($query);
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    <h4 class="page-title">New Review</h4>
                                        ...
                                        <form id="department_form" class="needs-validation">
											<div class="row mb-3">
												<div class="col-md-12">
													...
													<select name="agree_with_assestment" class="form-control" onchange="action_acr_review(this.value,<?= $record['id']; ?>)">
														<option value="">---</option>
		...
		$.ajax({
			...
			success:function(data){
				...
				if(data=='Success'){
					...
					setTimeout(function() {
						window.location.replace("appraisal-list");
					}, 1000); 
				}else if(data=='Error'){
